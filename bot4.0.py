...
def recur(update, context, msg):
	...
	if msg_lower == "e":
		sendVideo(update, context, 'eeeee.mp4')
...
def handleText(update, context):
	...
	if auto_reply:
		for i in range(len(video_strong_tags)):
...
def update_video_names():
	...
	#Mouhahaha
	video_names_out=sorted([i.split('>')[0][9:-1] for i in unquote(requests.get(dataServerAddress).text).split("<img src=\"/__ovh_icons/movie.gif\" alt=\"[VID]\"> ")[1:-2]])
