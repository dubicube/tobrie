...
def GPT_checkChatIDExists(chatID):
    if not chatID in gpt_convData:
        gpt_convData[chatID] = (False, GPT_SYSTEM_PROMPT)
...
def GPT_startConvMode(contextual_bot, sh_core):
    ...
    if len(prompt) != 0:
        gpt_convData[chatID] = (False, GPT_SYSTEM_PROMPT + [{"role": "system", "content": prompt}])
    else:
        gpt_convData[chatID] = (True, GPT_SYSTEM_PROMPT)
