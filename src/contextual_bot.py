    ...
    def outputMessages(self):
        ...
        if len(self.reply_queue_pic) > 0:
            ...
            for (type, obj, proba) in self.reply_queue_pic:
                proba_total += proba
            target_proba = random.randint(
                0,
                proba_total
                + (100 * len(self.reply_queue_pic) - proba_total)
                // len(self.reply_queue_pic)
                - 1,
            )
...
class DiscordBot(ContextualBot):
    ...
    async def outputMessages(self):
        ...
        for (type, obj, proba) in self.reply_queue:
            if (
                type == ContextualBot.TEXT
                or type == ContextualBot.VIDEO
                or type == ContextualBot.ANIMATION
            ):
                await self.message.channel.send(obj)
            if (
                type == ContextualBot.DOCUMENT
                or type == ContextualBot.IMAGE
                or type == ContextualBot.AUDIO
            ):
            ...
            if type == ContextualBot.CHAINED_STICKERS:
                for o in obj:
                    ...
                    await self.message.channel.send(
                        file=DiscordFile("temp/sticker.webp")
                    )
        super().clearQueue()
