...
def parseDateTime(date, time):
    ...
    try:
        ...
        if len(d) == 3:
            ...
            fuck = getDateTime(
                a, b, c, parsed_time.hour, parsed_time.minute, parsed_time.second
            )
...
def addEvent(data, date, time, text):
    ...
    return (
        0,
        getNextTrig(
            datetime.datetime.today(), targetDay, targetMonth, targetYear, targetTime
        ),
        data,
    )
...
def getNextTrig(today, targetDay, targetMonth, targetYear, targetTime):
    if targetMonth == -1 and targetYear == -1:
        ...
        r = r.replace(
            ...
            microsecond=targetTime.microsecond,
        )
    ...
    if targetYear == -1:
        ...
        if r == None:  # looking for bysextil year or error
            return getNextTrigBysextil(
                today.year, targetDay, targetMonth, targetYear, targetTime
            )
        elif r > today:
            return r
        else:
    ...
    for l in data.split("\n"):
        ...
        if len(m) == 3:
            ...
            if error_code == 0:
                (targetDay, targetMonth, targetYear) = targetDate
                (kr, kdt) = keepFun(
                    today, targetDay, targetMonth, targetYear, targetTime
                )
                if kr != -1:
                    newData += l + "\n"
                if kr == 1:
...
def startEventThread(eventList, actionFun=defaultActionFun):
    ...
    threading.Thread(
        target=eventThread, args=(threadingEvent, eventList, actionFun)
    ).start()
