...
def update_video_names():
    ...
    # Mouhahaha
    video_names_out = sorted(
        [
            i.split(">")[0][9:-1]
            for i in urllib.parse.unquote(requests.get(dataServerAddress).text).split(
...
def update_video_names_command(update, context):
    ...
    context.bot.send_message(
        ...
        text="Sticker map mise à jour (" + str(len(sticker_map_regex)) + " stickers)",
    )
    context.bot.send_message(
        chat_id=update.message.chat_id,
        text="Text map mise à jour (" + str(len(text_map_regex)) + " textes)",
    )
    context.bot.send_message(
        chat_id=update.message.chat_id,
        text="Video map mise à jour (" + str(len(video_map_regex)) + " vidéos)",
    )
    update_video_names()
    global video_names_out
    context.bot.send_message(
        chat_id=update.message.chat_id,
        text="Vidéos mises à jours (" + str(len(video_names_out)) + " vidéos).",
    )
...
def inlinequery(update, context):
    context.bot.send_message(
        ...
        text="["
        + ", "
        + str(update.inline_query.from_user.first_name)
        + ", "
    ...
    if txt[0] == "z":
        ...
        results = [
            InlineQueryResultVideo(
                uuid4(),
                dataServerAddress + vname.replace(" ", "%20"),
                "video/mp4",
                thumbnailsServerAddress + vname[:-3].replace(" ", "%20") + "jpg",
                vname,
            )
            for vname in r
        ]
...
def list(update, context):
    ...
    if update.message.chat_id == update.message.from_user.id:
        ...
        if len(update.message.text) > 6:
            list_out = getResults(
                update.message.text[6:].lower().split(" "), video_names_out, 9999999
            )
        ...
        context.bot.send_message(
            chat_id=update.message.chat_id,
            text="Le /list est interdit dans les groupes",
        )
...
def dico(update, context):
    ...
    if update.message.chat_id == update.message.from_user.id:
        ...
        for i in range(len(d) // 100):
            context.bot.send_message(
                chat_id=update.message.chat_id,
                text=", ".join(d[i * 100 : (i + 1) * 100]),
            )
        context.bot.send_message(
            chat_id=update.message.chat_id,
            text=", ".join(d[(int(len(d) // 100)) * 100 :]),
        )
    else:
        context.bot.send_message(
            chat_id=update.message.chat_id,
            text="Le /dico est interdit dans les groupes",
        )
...
def meme(contextual_bot, sh_core):
    sh_core.notifConsole(contextual_bot)
    memes = [
        f for f in os.listdir(memePath) if os.path.isfile(os.path.join(memePath, f))
    ]
...
def getAlie(contextual_bot, sh_core):
    ...
    product = sh_core.remote_service.sendToClient(
        "1" + contextual_bot.getText()[5:]
    ).split("///")
    if len(product) != 2:
...
def rapport(update, context):
    ...
    if (
        ...
        context.bot.send_message(
            ...
            + " minutes et "
            + str((sec % 3600) % 60)
            + " secondes pour finir le rapport (ou le commencer...)",
        )
    else:
        ...
        context.bot.send_message(
            ...
            + " minutes et "
            + str((sec % 3600) % 60)
            + " secondes pour finir le rapport avec "
            + str(24 * -reste.days)
            + "h de retard",
        )
    ...
    if len(results) < 10:
        for l in results:
            contextual_bot.reply(
                ...
                + ("Nom: " + l[1] + "\n" if l[1] != "" else "")
                + ("Emplacement: " + l[2] + "\n" if l[2] != "" else "")
                + ("Caractéristique: " + l[3] if l[3] != "" else ""),
            )
    else:
...
def deprecatedCommand(contextual_bot, sh_core):
    ...
    contextual_bot.reply(
        ContextualBot.VIDEO, dataServerAddress + "tintin_c'est_con_capitaine.mp4"
    )
    contextual_bot.outputMessages()
    contextual_bot.reply(
        ContextualBot.TEXT,
        "Consultez l'aide (/help) pour découvrir les commandes alternatives",
    )
    contextual_bot.outputMessages()
...
def getCitation(contextual_bot, sh_core):
    contextual_bot.reply(
        ContextualBot.TEXT, getRandomLine(open(mapPath + "citations", "r").read())
    )
...
def generic_handle_text(contextual_bot, sh_core):
    ...
    if msg[0] == "/":
        for (fun_txt, fun) in commands:
            if msg[1:].startswith(fun_txt) and (
                len(msg[1:]) == len(fun_txt) or msg[len(fun_txt) + 1] == " "
            ):
...
def addMusic(contextual_bot, sh_core):  # /addm
    ...
    if not updated:
        contextual_bot.reply(
            ContextualBot.TEXT,
            str(size) + "  videos added from local cache\nType /fetch to update cache",
        )
    else:
        contextual_bot.reply(ContextualBot.TEXT, str(size) + "  video(s) added")
    contextual_bot.reply(
        ContextualBot.TEXT, str(len(musicQueue.queue)) + "  video(s) in total"
    )
    if "shuffle" in data:
        musicQueue.shuffle()
...
def updateMusic(contextual_bot, sh_core):  # /fetch
    if musicQueue.playlist == None:
        ...
        contextual_bot.reply(
            ContextualBot.TEXT,
            "Playlist updated ("
            + str(musicQueue.playlist.size - old_size)
            + " new videos)",
        )
...
def infoMusic(contextual_bot, sh_core):  # /queue
    contextual_bot.reply(
        ContextualBot.TEXT,
        str(len(musicQueue.queue)) + "  video(s)\nCursor: " + str(musicQueue.cursor),
    )
...
async def discordPlayMic(message):
    ...
    source = discord.FFmpegAudio(
        source='audio="Réseau de microphones (Realtek High Definition Audio)"',
...
def telegram_new_member(update, context):
    ...
    for m in members:
        ...
        if m.id == id_bot:
            ...
            contextual_bot.reply(
                ContextualBot.VIDEO, dataServerAddress + "eddy_malou_bonjour.mp4"
            )
            contextual_bot.outputMessages()
            contextual_bot.reply(
                ContextualBot.TEXT,
...
def telegram_periodic_thread(update, context):
    if update.message.from_user.id == super_admin:
        ...
        if arg == "c":
            context.bot.send_message(
                update.message.chat_id, "Watchdog: " + str(periodic_thread_watchdog)
            )
...
commands = [
    ("di", deprecatedCommand),
    ("video", deprecatedCommand),
    ("config", configureParameters),
    ("proba", configureProba),
    ("on", setAutoReplyOn),
    ("off", setAutoReplyOff),
    ("find", find),
    ("info", info),
...
def main():
    ...
    if TELEGRAM_ENABLE:
        ...
        dp.add_handler(
            MessageHandler(Filters.status_update.new_chat_members, telegram_new_member)
        )
